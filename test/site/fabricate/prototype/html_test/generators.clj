...
(def nouns ["steel" "referent" "microscope" "antenna" "star" "harp" "electron"])
  ...
  (m/schema
   [:orn [:bool :boolean]
    [:decimal
     [:double
      {:gen/gen (gen/double*
                 {:infinite? false :NaN? false :min -1000 :max 1000})}]]
    [:integer [:int {:gen/gen (gen/large-integer* {:min -1000 :max 1000})}]]
...
(def global-attributes-bounded
  (->
    ...
    (mu/update :class #(mu/update-properties % assoc :gen/elements nouns))
    (mu/update :id
  ...
  ([{:keys [outer-tags inner-tags contents-gen max-breadth max-height]
   ...
   (gen'/bounded-recursive-gen
    (fn [inner]
      (gen/one-of [inner
                   (gen/let [i     inner
                             outer (hiccup-base-elems {:tags outer-tags
                                                       :contents-gen
                                                       contents-gen})]
                     (conj outer i))]))
...
(def element
  ...
  (gen/such-that html/element? (hiccup-recursive-elems)))


(comment
  (gen/sample (gen/such-that html/element?
                             (hiccup-recursive-elems {:outer-tags
                                                      html/block-level-tags})))
  (gen/sample element 20))
