                   ...
                   (cond
                     (and (string? h) (some? (re-find re h)))
                       (let [[hh & tt] (string/split h re)
                         ...
                         (cond
                           (or (empty? hh) (re-matches (re-pattern "\\s+") hh))
...
(defn parse-paragraphs
  ...
  ([form
    {:keys [paragraph-pattern default-form current-paragraph?],
     :or
       {paragraph-pattern #"\n\n", default-form [:p], current-paragraph? false},
     :as opts}]
                   ...
                   (let [previous (peek acc)
                     ...
                     (cond
                       ...
                       (and (sequential? next)
                            ...
                            (html/phrasing? next))
                         (conj r-acc
                               (conj previous (parse-paragraphs next opts)))
  ...
  (cond (:k node) "keyword"
        ...
        (contains? node :value) (let [v (:value node)]
                                  (cond (number? v) "number"
                                        ...
                                        :else (name (tag node))))
        :else
          (println (tag node) (keys node) (sexpr node) (type (sexpr node)))))
...
(defn- fn-node->hiccup
  ...
  (let [contents (:children node)
        ...
        r (cond (= 0 (count params)) {}
                ...
                :else (into {}
                            (map-indexed
                              (fn [ix sym] [sym (symbol (str "%" (inc ix)))])
                              params)))
...
(defmethod node->hiccup :list
  ...
  (if (fn-list? node)
    ...
    (apply span
      "list"
      (span "open-paren" "(")
      (conj (mapv node->hiccup (:children node)) (span "close-paren" ")")))))
