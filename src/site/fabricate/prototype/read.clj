...
(defn parsed-form->expr-map
  ...
  (let [[t form-or-ctrl? form?] parsed-form
        ...
        m (with-meta (merge {:expr-src
                               (if (vector? form-or-ctrl?) form? form-or-ctrl?),
                             :display false}
                            (if (or (:error read-results)
                                    (::parse-error read-results))
...
(defn extended-form->form
  ...
  (let [delims (str open close)
    ...
    (with-meta (cond (= delims "[]")
                       (apply conj [] (concat parsed-front-matter forms))
    ...
    [:schema
     {:registry
        {::txt [:tuple {:encode/get {:leave second}} [:= :txt] :string],
         ::form [:or
                 [:tuple {:encode/get {:leave parsed-form->expr-map}} [:= :expr]
                  [:tuple [:= :ctrl] [:enum "=" "+" "+="]] :string]
                 [:tuple {:encode/get {:leave parsed-form->expr-map}} [:= :expr]
                  :string]],
         ::extended-form [:tuple {:encode/get {:leave extended-form->form}}
                          [:= :extended-form] [:enum "{" "[" "("] :string
                          [:cat [:= :form-contents]
                           [:*
                            [:or [:ref ::txt] [:ref ::form]
                             [:ref ::extended-form]]]] [:enum "}" "]" ")"]]}}
...
(defn render-src
  ...
  ([src-expr rm-do?]
   (let [exp (if (and rm-do? (seq? src-expr) (= (first src-expr) 'do))
               (second src-expr)
               src-expr)]
...
(defn get-file-metadata
  ...
  (let [wd (-> "."
    ...
    {:site.fabricate.file/input-filename
       (if (= "" local-subdir) fname (str local-subdir "/" fname)),
